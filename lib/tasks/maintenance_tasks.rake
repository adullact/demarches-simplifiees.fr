...
namespace :maintenance_tasks do
  ...
  # Use with `bin/rails "maintenance_tasks:generate_migrations[20260527]"`
  desc "Create a migration for each maintenance task"
  task :generate_migrations, [:from_timestamp] => :environment do |_task, args|
    from_timestamp = args[:from_timestamp].presence || 0
    ...
    Dir.glob(glob).each do |task_filename|
      ...
      next unless task_timestamp.to_i > from_timestamp.to_i
...
class #{task_name.camelize} < ActiveRecord::Migration[8.0]
